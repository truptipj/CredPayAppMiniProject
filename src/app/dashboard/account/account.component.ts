...
console.log(this.editCreditCard);
      ...
      this.addCardsForm = this.fb.group({
        ...
        expirationDate: [(this.editCreditCard && this.editCreditCard.expirationDate.substring(0,10)) || '', [Validators.required]],
    ...
    addUpdateCard(){
      if(!this.isUpdate) {
        ...
        cardReq.cardDetailId = this.editCreditCard.cardDetailId;
         this.cardService.updateCard(this.editCreditCard.cardDetailId,cardReq).subscribe((res)=>{
           if(res) {
             this.router.navigate(['/user/cards']);
           }
         })
  ...
  uniqueCardNumber(){
    ...
    this.isDuplicateCardNo = this.allCards.some(item => item.cardNumber === this.addCardsForm.value.cardNumber);
